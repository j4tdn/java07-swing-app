...
public class Bt02 extends JFrame{
    ...
    private void initComponent(){
       ...
       setSize(600, 500);
		...
		lbInput.setBounds(80, 50, getPreWidth(lbInput) + 30, getPreHeight(lbInput));
		add(lbInput);

		lbPerform.setFont(textFont);
		lbPerform.setText("Thực hiện: ");
		lbPerform.setBounds(87, 20 + getPreWidth(lbInput), getPreWidth(lbPerform)+30, getPreHeight(lbPerform));
		add(lbPerform);

		lbResult.setFont(textFont);
		lbResult.setText("Kết quả: ");
		lbResult.setBounds(101, 110 + getPreHeight(lbInput) * 2, getPreWidth(lbResult)+ 30, getPreHeight(lbResult));
		add(lbResult);
		...
		lbError.setBounds(110 + getPreWidth(lbInput), 20, getPreWidth(lbError)+30, getPreHeight(lbError));
		add(lbError);
